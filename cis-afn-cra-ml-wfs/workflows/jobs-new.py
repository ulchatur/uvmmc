...
def regression_training():
    ...
    del hub_data; gc.collect()  # **ADDED LINE 2**
    spoke_df = etl_spoke()
    (adjusted_df, latest_year, latest_quarter) = apply_manual_adjustments(df_ml=cleaned_df, df_spoke=spoke_df)
    del spoke_df; gc.collect()  # **ADDED LINE 3**
    modeling_df = build_ml_data(adjusted_df, latest_year, latest_quarter)
    final_model_df = apply_overrides_adjustments(df=modeling_df, df_ml=cleaned_df, df_spoke=None, latest_year=latest_year, latest_quarter=latest_quarter)
    ...
    del final_model_df; gc.collect()  # **ADDED LINE 4**
...
def classification_training():
    ...
    hub_data = get_raw_data_files()
    cleaned_df = initial_etl(hub_data)
    del hub_data; gc.collect()  # **ADDED LINE 5**
    spoke_df = etl_spoke()
    (adjusted_df, latest_year, latest_quarter) = apply_manual_adjustments(df_ml=cleaned_df, df_spoke=spoke_df)
